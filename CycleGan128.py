...
def build_generator(input_image, name):
    ...
    with tf.variable_scope(name):
        ...
        #********************************************DOWNSAMPLING*******************************************************
        #(128,128,3)
        x = tf.layers.conv2d(inputs = input_image, filters=32, kernel_size=7, strides=1, padding="same")   
        x = tf.contrib.layers.layer_norm(inputs=x, begin_norm_axis=1)  
        x = tf.nn.relu(x) 
        #(128,128,32)
        x = tf.layers.conv2d(inputs= x, filters=64, kernel_size=3, strides=2, padding="same") #(64,64,64) 
        x = tf.contrib.layers.layer_norm(inputs=x, begin_norm_axis=1)
        x = tf.nn.relu(x)
        #(64,64,64)
        x = tf.layers.conv2d(inputs= x, filters=128, kernel_size=3, strides=2, padding="same") #(32,32,128)       
        x = tf.contrib.layers.layer_norm(inputs=x, begin_norm_axis=1)
        x = tf.nn.relu(x)
        #(32,32,128)
         #********************************************RESIDUAL*******************************************************
        ...
        #********************************************UPSAMPLING*******************************************************
        #(32,32,128)  
        x = tf.layers.conv2d_transpose(inputs= x, filters=64, kernel_size=3, strides=2, padding='same', use_bias=False) #(64,64,64)       
        x = tf.contrib.layers.layer_norm(inputs=x, begin_norm_axis=1)
        x = tf.nn.relu(x)
        #(64,64,64) 
        x = tf.layers.conv2d_transpose(inputs= x, filters=32, kernel_size=3, strides=2, padding='same', use_bias=False) #(128,128,32)        
        x = tf.contrib.layers.layer_norm(inputs=x, begin_norm_axis=1)
        x = tf.nn.relu(x)
        #(128,128,32)  
...
if __name__ == '__main__':
    ...
    #dataset_type = "maps/"
    #dataset = "/content/drive/My Drive/VisionPerception/datasets/" + "dataset_type"

    dataset_type = "monet/"    
    dataset = "/content/drive/My Drive/VisionPerception/datasets/monet2photocomplete/"
    #dataset = "facades/"
    #dataset = "/content/drive/My Drive/VisionPerception/datasets/facades/"
    #dataset_type = "facades1/"    
    #dataset = "/content/drive/My Drive/VisionPerception/datasets/" + "dataset_type"
    dataset_type = "ukiyoe/"    
    dataset = "/content/drive/My Drive/VisionPerception/datasets/ukiyoe2photo/"
